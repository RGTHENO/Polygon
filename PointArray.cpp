...
Point* PointArray::get(const int pos){ /**  Esta funcion es recomendable para objectos que deseamos que sean modificables
													i.e para non-const pointArray objects por lo cual podriamos querer permitir
													* modificar los objetos "Point" contenidos.
													**/
	if(pos>=0 && pos<size){
		return  arreglo+pos;
	}
...
const Point* PointArray::get(const int pos)const { /** Retorna un puntero de solo lectura para objetos
															pointArray constantes**/
	if(pos>=0 && pos<size){
		return  arreglo+pos;
	}
